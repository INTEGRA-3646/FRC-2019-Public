...
public class CollectVelocityData implements Action {
    ...
    public void update() {
        ...
        mVelocityData.add(new DriveCharacterization.VelocityDataPoint(
                (Math.abs(mDrive.getLeftVelocityNativeUnits()) + Math.abs(mDrive.getRightVelocityNativeUnits())) / mDrive.getDriveEncoderPPR() * Math.PI * 10, //convert velocity to radians per second
