package com.team3646.lib.trajectory.timing;

import com.team3646.lib.geometry.State;
import com.team3646.lib.trajectory.DistanceView;
...
public class TimingUtil {
    ...
    public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
        ...
        // parameterized (though we may adjust the velocity downwards during the backward pass). We wish to find an
        // acceleration that is admissible at both the start and end state, as well as an admissible end velocity. If
        // there is no admissible velocity or acceleration, we set the end velocity to the state's maximum allowed
        ...
        for (int i = 0; i < states.size(); ++i) {
            ...
            while (true) {
                ...
                final double global_max_velocity = Math.sqrt(predecessor.max_velocity * predecessor.max_velocity +
                                                             2.0 * predecessor.max_acceleration * ds);
                constraint_state.max_velocity = Math.min(max_velocity, global_max_velocity);
                ...
                // Now enforce all acceleration constraits.
                for (final TimingConstraint<S> constraint : constraints) {
        ...
        for (int i = states.size() - 1; i >= 0; --i) {
            ...
            while (true) {
                ...
                // Now check all acceleration contraints with the lower max velocity
                for (final TimingConstraint<S> constraint : constraints) {
